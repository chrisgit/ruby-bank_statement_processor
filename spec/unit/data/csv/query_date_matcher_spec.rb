  ...
  describe 'QueryDateMatcher' do
    describe 'start and end dates are nil' do
      it 'permits all dates' do
        date_query_matcher = BankStatements::MemoryQuery::QueryDateMatcher.new(nil,nil)
    ...
    describe 'start date specified and end date is nil' do
      describe 'date is before start date' do
        it 'returns false' do
          date_query_matcher = BankStatements::MemoryQuery::QueryDateMatcher.new(Date.parse('01/04/2017'),nil)
          @transaction.date = Date.parse('21/03/2017')
    ...
    describe 'start date is nil and end date is specified' do
      describe 'date is before end date' do
        it 'returns true' do
          date_query_matcher = BankStatements::MemoryQuery::QueryDateMatcher.new(nil, Date.parse('01/04/2017'))
          @transaction.date = Date.parse('21/03/2017')
          expect(date_query_matcher.match(@transaction)).to be true
        end
      end
    ...
    describe 'start and end dates are specified' do
      describe 'date is before start date' do
        it 'returns false' do
          date_query_matcher = BankStatements::MemoryQuery::QueryDateMatcher.new(Date.parse('01/03/2017'), Date.parse('30/04/2017'))
